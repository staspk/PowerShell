...
class MyRuntime {
    ...
    MyRuntime() {
        $this._GLOBALS_FILE = $this._GLOBALS_FILE;
    ...
    NewCommand($path, [string]$command) {
        ...
        $this.commands = [MyRuntime]::LoadEnvFileIntoMemory($this._COMMANDS_FILE)
        PrintGreen "NewCommand(" $false; PrintItalics $command DarkGreen; PrintGreen ") created on `$path: $path"
    }
    ...
    RunDefaultCommand() {
        ...
        if(-not($command)) {
            PrintRed "No Default Command Assigned to `$PWD";  RETURN;  }
