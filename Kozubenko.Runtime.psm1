...
class MyRuntime {
    ...
    [void] Help([string]$moduleName) {
        $longest_func_signature = 0

        foreach ($module in $this.modules) {
            if($module.moduleName -match $moduleName) {
                if($module.longest_func_signature -gt $longest_func_signature) { $longest_func_signature = $module.longest_func_signature }
            }
        }
        ...
        foreach ($module in $this.modules) {
            if($module.moduleName -match $moduleName) {
                $module.Print($longest_func_signature)
            }
        }
    }
